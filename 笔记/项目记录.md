...
16. 分支操作,查看所有分支：`git branch` 创建分支：`git branch xxx` 切换分支：`git checkout xxxx` 创建分支同时切换分支：`git checkout -b xxxx` 删除分支：`git bransh -D xxxx`  `git fetch`：获取所有的分支


##   命令知识点

1. `ldd + 可执行二进制文件`：可以查看引用的库
